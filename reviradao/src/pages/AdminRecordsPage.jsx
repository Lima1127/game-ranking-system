      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-3">
          <div className="flex items-center gap-3 flex-wrap">
            <h3 className="text-2xl font-black text-slate-900">{item.gameName}</h3>
          ...
          {item.proofId && (
            <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
              <div className="text-sm font-semibold text-slate-700">Anexo</div>
              {item.proofContentType?.startsWith('image/') && (
                <img
                  src={proofUrl}
                  alt={`Anexo de ${item.gameName}`}
                  className="max-h-52 rounded-lg border border-slate-200 bg-white object-contain"
                />
              )}
              <a
                href={proofUrl}
...
export default function AdminRecordsPage() {
  ...
  if (isLoading) {
    return <div className="text-slate-600">Carregando painel administrativo...</div>;
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10">
      <div>
        <h1 className="text-4xl font-black text-slate-900 mb-2">Gestao Administrativa</h1>
      ...
      <section className="space-y-4">
        <div>
          <h2 className="text-2xl font-black text-slate-900">Registros aprovados</h2>
          <p className="text-sm text-slate-500">Excluir daqui remove os pontos atuais do ranking para aquela conclusao.</p>
        </div>

        {approvedItems.length === 0 ? (
          <div className="rounded-2xl bg-white p-6 text-slate-500 shadow-sm">Nenhum registro aprovado encontrado.</div>
        ) : (
      ...
      <section className="space-y-4">
        <div>
          <h2 className="text-2xl font-black text-slate-900">Historico de solicitacoes</h2>
          <p className="text-sm text-slate-500">Use esta area para limpar solicitacoes pendentes ou canceladas que nao fazem mais sentido manter.</p>
        </div>

        {requestHistory.length === 0 ? (
          <div className="rounded-2xl bg-white p-6 text-slate-500 shadow-sm">Nenhuma solicitacao historica encontrada.</div>
        ) : (
